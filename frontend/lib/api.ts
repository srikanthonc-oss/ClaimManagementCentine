...
export const api = {
  ...
  claims: {
    ...
    getEobImageUrl: (id: string) =>
      request(`/api/claims/${id}/eob-image-url`),
    amendPosting: (id: string, lineAmendments: any[]) =>
      request(`/api/claims/${id}/amend-posting`, { method: 'POST', body: JSON.stringify({ line_amendments: lineAmendments }) }),
